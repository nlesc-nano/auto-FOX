...
The root mean squared displacement (RMSD) represents the average displacement of a 
set or subset of atoms as a function of time or, equivalently, moleculair indices
in a MD trajectory.
...
Given a trajectory, ``mol``, stored as a *MultiMolecule* object, the RMSD can be
calculated with the following command: ``rmsd = mol.init_rmsd(atom_subset=None)``.
...
Given a trajectory, ``mol``, stored as a *MultiMolecule* object, the RMSF can be
calculated with the following command: ``rmsf = mol.init_rmsf(atom_subset=None)``.
...
In the above two examples ``atom_subset=None`` was used an optional keyword, one which allows 
one to customize for which atoms the RMSD & RMSF should be calculated and how the results 
are distributed over the various columns.
...
It should be noted that lists and/or tuples can be interchanged for any other iterable container (*e.g.* a Numpy_ array), as long as the iterables elements can be accessed by their index.
